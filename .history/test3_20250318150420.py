...
def adam_optimization(
    ...
    """
    Simulate CDTools' model optimization process [^2][^4]
    - Supports multi-stage learning rate adjustment
    """
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in jnp.array_split(perm, num_samples//batch_size):
            ...
            initial_params_batch = initial_params[batch_idx]
            probe_batch = probe[batch_idx]
            
            # Automatic differentiation to compute gradients
            grad_fn = jax.grad(lambda p: loss_function(p, probe_batch, batch_meas))
            grads = grad_fn(initial_params_batch)
            
            # Update parameters
            updates, opt_state = optimizer.update(grads, opt_state)
            initial_params = optax.apply_updates(initial_params_batch, updates)
...
update_obj = adam_optimization(obj, data, probe, 0.5, 500, batch_size=50)

update_obj_1 = adam_optimization(update_obj, data, probe, 0.05, 750, batch_size=75)

update_obj_2 = adam_optimization(update_obj_1, data, probe, 0.005, 1000, batch_size=100)
