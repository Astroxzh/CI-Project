...
m, n = probe.shape[1], probe.shape[2]  # Assume probe is pre-calibrated with shape (2, m, n)
f = 2
m_d, n_d = m // f, n // f
init_obj_low = jnp.ones((m_d, n_d), dtype=jnp.complex64)  # Initialize low-resolution object
background = jnp.ones_like(data) * 0.01  # Match measured data shape

update_obj_low = adam_optimization(init_obj_low, data, background, probe, 0.1, 200)

plt.imshow(jnp.angle(update_obj_low))
plt.show()